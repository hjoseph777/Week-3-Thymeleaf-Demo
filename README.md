...
```bash
git clone https://github.com/hjoseph777/Thymeleaf-Demo.git
cd Thymeleaf-Demo
```
...
Wait for the console to say something like:
...
### 4. If You Hit a Build Error

If you see `error: release version 1.8 not supported`, it means your JDK is too new (JDK 21 dropped Java 8 target support). The fix is already in this repo — the `pom.xml` explicitly sets:
...
Make sure your `JAVA_HOME` points to JDK 17 or 21. Check with:
